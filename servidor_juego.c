...
void Inicializar(TablaPartidas tabla){
	...
	for(i=0;i<100;i++){
		...
		tabla[i].distancias[0]= -1;
		tabla[i].distancias[1]= -1;
		tabla[i].distancias[2]= -1;
		tabla[i].distancias[3]= -1;
		strcpy(tabla[i].Jugador1.nombre,"F");
		strcpy(tabla[i].Jugador2.nombre,"F");
		strcpy(tabla[i].Jugador3.nombre,"F");
		strcpy(tabla[i].Jugador4.nombre,"F");
	}
}
...
int BorrarPartida(TablaPartidas tabla, int indice){
	...
	//elimina una partida cuando ha finalizado (inicializamos una partida concreta)
	//F indica que no hay un jugador usando el string
	...
	strcpy(tabla[indice].Jugador1.nombre,"F");
	strcpy(tabla[indice].Jugador2.nombre,"F");
	strcpy(tabla[indice].Jugador3.nombre,"F");
	strcpy(tabla[indice].Jugador4.nombre,"F");
	tabla[indice].distancias[0]= -1;
	tabla[indice].distancias[1]= -1;
	tabla[indice].distancias[2]= -1;
	tabla[indice].distancias[3]= -1;
}

int BuscarHost(TablaPartidas tabla, char emisor[20]){
	//busca la partida que ha creado el emisor (host)
	int encontrado = 0;
	int i = 0;
	while((i<100)&&(!encontrado)){
...
void EscribePartida(TablaPartidas tabla, int indice){
	printf("********************Función EscribePartida\n");
	printf("Indice: %d\n", indice);
	printf("Estado: %d\n", tabla[indice].estado);
	printf("Num jugadores: %d\n", tabla[indice].num_jugadores);
	printf("Jugador1: %s\n", tabla[indice].Jugador1.nombre);
	printf("Jugador2: %s\n", tabla[indice].Jugador2.nombre);
	printf("Jugador3: %s\n", tabla[indice].Jugador3.nombre);
	printf("Jugador4: %s\n", tabla[indice].Jugador4.nombre);
	printf("********************Fin función EscribePartida\n");
}
...
int AnadirConectado(ListaConectados *lista, char nombre[20], int socket) {
	...
	if (lista->num <  MAX) {
		...
		lista->conectados[lista->num].jugando = 0;
...
int PonerJugando(ListaConectados *lista, int socket) {
	// -1 si no encuentra ese jugador
	int found = 0;
	int i = 0;
	while (i < lista->num && !found) {
		if (lista->conectados[i].socket == socket) {
			lista->conectados[i].jugando = 1;
			found = 1;
		}
		i++;
	}
...
int QuitarJugando(ListaConectados *lista, int socket) {
	// -1 si no encuentra ese jugador
	int found = 0;
	int i = 0;
	while (i < lista->num && !found) {
		if (lista->conectados[i].socket == socket) {
			lista->conectados[i].jugando = 0;
			found = 1;
		}
		i++;
	}
...
/*	cadena[strlen(cadena)-1] = '\0';*/
	...
	printf("%d",lista->num);
	for (int i = 0; i < lista->num; i++){
		sprintf(cadena, "%s%s*%d,", cadena, lista->conectados[i].nombre,lista->conectados[i].jugando);
		printf("%s\n",lista->conectados[i].nombre);
	}
	cadena[strlen(cadena)-1] = '\0';
}

void Cadena2(ListaConectados *lista, char cadena[200]) {
	// numero de jugadores + nombre de todos esos jugadores, separados todos esos datos por una coma
	sprintf(cadena, "%d,", lista->num);
	printf("%d",lista->num);
	for (int i = 0; i < lista->num; i++){
...
void Codigo2_14(char buff2[512], char nombre[20]) {
	//Usuarios contra los que ha jugado un jugador
	//2/14/1 si no hay rivales
	//2/14/0/nombre1,nombre2...
	int err;
	MYSQL_RES *resultado;
	MYSQL_ROW row;
	char consulta [500];
	sprintf (consulta, "SELECT DISTINCT (Jugador.Nombre) FROM (Jugador,Participacion) WHERE Participacion.Id_J = Jugador.Id AND Participacion.Id_P IN (SELECT DISTINCT (Participacion.Id_P) FROM (Jugador,Participacion) WHERE Jugador.Nombre = '%s' AND Participacion.Id_J = Jugador.Id)", nombre);
	...
	else
	{
		...
		sprintf(buff2, "2/14/0/%s", jugadores );
	}	
	printf ("%s\n", buff2);
}
	...
	if(strcmp(resultado,"si")==0){
		...
		if(tablaPartidas[indice].estado == 1){// Si la partida aun no ha empezado ponemos el jugador
			...
			if(aux == 0){
				...
				PonerJugando(&listaConectados,sock_conn);
				PonerJugando(&listaConectados,socket_receptor);
				write (socket_receptor, buff2, strlen(buff2));
			}
			else if(aux == -1){
				sprintf(buff2, "2/6/1"); 
...
void Codigo2_8 (char usuario[20], char buff2[512], int sock_conn, char texto[200]){
	...
	for (int j=0; j < listaConectados.num; j++) {
		write(listaConectados.conectados[j].socket,buff2,strlen(buff2));//Cambiar los sockets por los sockets de la lista
	}
	printf ("\n%s\n", buff2);
}

void Codigo2_9(char buff2[512], int sock_conn, int indicePartida, char idCoche[20], char posX[20], char posY[20]){
	//Enviar posición al resto de jugadores de nuestra partida
	sprintf(buff2,"2/9/%s,%s,%s", idCoche, posX, posY);
	tablaPartidas[indicePartida].distancias[atoi(idCoche)-1] = atoi(posY);
...
void Codigo2_10(char buff2[512], int sock_conn, int cocheEliminado, int indicePartida){
	//Enviamos si nos chocamos para que los otros nos quiten de su pantalla
	strcpy(buff2,"2/10/");
	sprintf(buff2,"%s%d",buff2,cocheEliminado);
...
void Codigo2_11(char buff2[512], int sock_conn, int indicePartida){
	//Enviamos los resultados de la partida
	
	int idP = CrearPartida(indicePartida);
	sprintf(buff2,"2/11/%d,%s,%d,%s,%d,%s,%d,%s,%d,%d,",tablaPartidas[indicePartida].num_jugadores,tablaPartidas[indicePartida].Jugador1.nombre,tablaPartidas[indicePartida].distancias[0],tablaPartidas[indicePartida].Jugador2.nombre,tablaPartidas[indicePartida].distancias[1],tablaPartidas[indicePartida].Jugador3.nombre,tablaPartidas[indicePartida].distancias[2],tablaPartidas[indicePartida].Jugador4.nombre,tablaPartidas[indicePartida].distancias[3],idP);
	printf("%s\n", buff2);
	if(tablaPartidas[indicePartida].Jugador1.socket != -1){
		write(tablaPartidas[indicePartida].Jugador1.socket,buff2,strlen(buff2));
		QuitarJugando(&listaConectados, tablaPartidas[indicePartida].Jugador1.socket);
		printf("%s\n", buff2);
	}
	if(tablaPartidas[indicePartida].Jugador2.socket != -1){
		write(tablaPartidas[indicePartida].Jugador2.socket,buff2,strlen(buff2));
		QuitarJugando(&listaConectados, tablaPartidas[indicePartida].Jugador2.socket);
		printf("%s\n", buff2);
	}
	if(tablaPartidas[indicePartida].Jugador3.socket != -1){
		write(tablaPartidas[indicePartida].Jugador3.socket,buff2,strlen(buff2));
		QuitarJugando(&listaConectados, tablaPartidas[indicePartida].Jugador3.socket);
		printf("%s\n", buff2);
	}
	if(tablaPartidas[indicePartida].Jugador4.socket != -1){
		write(tablaPartidas[indicePartida].Jugador4.socket,buff2,strlen(buff2));
		QuitarJugando(&listaConectados, tablaPartidas[indicePartida].Jugador4.socket);
	}
	
	//Borrar partida de listaPartidas
	EscribePartida(tablaPartidas, indicePartida);
	BorrarPartida(tablaPartidas, indicePartida);
	EscribePartida(tablaPartidas, indicePartida);

}
int CrearPartida(int indicePartida) {
	//returna el id de la partida en la BBDD
	int id=0;
	char comanda[200];
	int err;
	MYSQL_RES *resultado;
	MYSQL_ROW row;
	//Miramos el id más grande
	strcpy(comanda, "SELECT MAX(Id) FROM Partida");
	err=mysql_query (conn, comanda);
	if (err!=0) {
		printf ("Error al consultar datos de la base %u %s\n",
				mysql_errno(conn), mysql_error(conn));
	...
	sprintf(comanda, "INSERT INTO Partida VALUES(%d,'2021/6/1')",id);
	err=mysql_query (conn, comanda);
	if (err!=0) {
		printf ("Error al modificar datos de la base %u %s\n",
				mysql_errno(conn), mysql_error(conn));
...
void Codigo2_12(char buff2[512], int sock_conn, char nombre[20]){
	...
	MYSQL_RES *resultado;
	MYSQL_ROW row;
	sprintf(comanda, "UPDATE Jugador SET Jugador.Nombre = 'Jugador borrado' WHERE Jugador.Nombre = '%s'",nombre);
	err=mysql_query (conn, comanda);
	sprintf(buff2, "2/12/0");
	if (err!=0) 
	{
		sprintf(buff2, "2/12/1");
		printf ("Error al consultar datos de la base %u %s\n",
				mysql_errno(conn), mysql_error(conn));
		exit (1);
	}
	BorrarConectado(&listaConectados, nombre);
	printf ("%s\n", buff2);
	write(sock_conn,buff2,strlen(buff2));
}
void Codigo2_13(char buff2[512], int sock_conn, char nombre[20], int idP, int posicion) {
	//returna el id de la partida en la BBDD
	char comanda[200];
	int err;
	MYSQL_RES *resultado;
	MYSQL_ROW row;
	//Miramos el id más grande
	sprintf(comanda, "SELECT Id FROM Jugador WHERE Jugador.Nombre = '%s'",nombre);
	pthread_mutex_lock( &mutex );
	err=mysql_query (conn, comanda);
	if (err!=0) {
		printf ("Error al consultar datos de la base %u %s\n",
	...
	printf("Max id partida: %d\n",atoi(row[0]));
	printf("Se añadirá: VALUES(%d,%d,%d)\n",idJ,idP,posicion);
	...
	sprintf(comanda, "INSERT INTO Participacion VALUES(%d,%d,%d)",idJ,idP,posicion);
	err=mysql_query (conn, comanda);
	if (err!=0) {
		printf ("Error al modificar datos de la base %u %s\n",
				mysql_errno(conn), mysql_error(conn));
...
void *AtenderCliente(void *socket){
	...
	while (terminar == 0){
		...
		if (form!=0 && codigo != 8 && codigo !=9 && codigo !=10 && codigo!=11){
			p = strtok( NULL, "/");
			strcpy (nombre, p);
		...
		if (form == 0){ //Nos desconectamos del servidor
			...
			if (codigo == 1) { // posición media
				...
				int cocheEliminado;
				p=strtok(NULL,"/");
				cocheEliminado = atoi(p);
				int indicePartida;
				p=strtok(NULL,"/");
				indicePartida = atoi(p);
				Codigo2_10(buff2, sock_conn, cocheEliminado, indicePartida);
			}
			else if(codigo==11){
				int indicePartida;
				p=strtok(NULL,"/");
				indicePartida = atoi(p);
				Codigo2_11(buff2, sock_conn, indicePartida);
			}
			else if(codigo==12){
				
				Codigo2_12(buff2, sock_conn, nombre);
			}
			else if(codigo==13){
				int idP;
				p=strtok(NULL,"/");
				...
				p=strtok(NULL,"/");
				posicion = atoi(p);
				Codigo2_13(buff2, sock_conn, nombre,idP,posicion);
			}
			else if (codigo == 14) {
				printf ("=========================================Codigo14\n");
				Codigo2_14(buff2, nombre);
				write (sock_conn,buff2, strlen(buff2));
			}
		}
		...
		if(((form==2)&&(codigo==6))||((form==2)&&(codigo==11))||((form==2)&&(codigo==12))){ //Cada vez que un usuario empieza a jugar
			Cadena2(&listaConectados, notificacion);
			sprintf(buff2, "2/4/%s", notificacion);
