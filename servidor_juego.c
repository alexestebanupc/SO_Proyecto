...
void Codigo1_1(char nombre[20], char password[20], char comanda[200], char buff2[512]) {
	int err;
	MYSQL_RES *resultado;
	MYSQL_ROW row;
...
void Codigo1_2(char nombre[20], char password[20], char comanda[200], char buff2[512]) {
...
void Codigo2_1(char nombre[20], char password[20], char comanda[200], char buff2[512]) {
...
void Codigo2_2(char nombre[20], char password[20], char comanda[200], char buff2[512]) {
...
void Codigo2_3(char nombre[20], char password[20], char comanda[200], char buff2[512]) {
...
void Codigo2_4(char comanda[200], char buff2[512]){
	
	char cadena[200];
	printf("hola%dadios",listaConectados.num);
	Cadena(&listaConectados, cadena);
	sprintf(buff2, "%s", cadena);
	printf ("%s\n", buff2);
	...
	while (terminar == 0){
		...
		if (form == 0){ //Nos desconectamos del servidor
			...
			if (codigo == 1) { // Iniciar sesión
				Codigo1_1(nombre, password, comanda, buff2);
				// Y lo enviamos
			...
			if (codigo == 2) { // Registrarse
				...
				Codigo1_2(nombre, password, comanda, buff2);
				// Y lo enviamos
				write (sock_conn,buff2, strlen(buff2));
			}
		}
			...
			if (codigo == 1) { // posición media
				...
				Codigo2_1(nombre, password, comanda, buff2);
				// Y lo enviamos
				write (sock_conn,buff2, strlen(buff2));
			}
			else if (codigo == 2) { //Jugador que ha ganado más veces
				
				Codigo2_2(nombre, password, comanda, buff2);
				// Y lo enviamos
				write (sock_conn,buff2, strlen(buff2));
			}
			else if (codigo == 3) {
				
				Codigo2_3(nombre, password, comanda, buff2);
				// Y lo enviamos
				write (sock_conn,buff2, strlen(buff2));
			}
			else if (codigo==4){
				
				Codigo2_4(comanda, buff2);
				// Y lo enviamos
				write (sock_conn,buff2, strlen(buff2));
			}
		}
